...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ $row->page ?? 'Service'  }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                        <li class="breadcrumb-item active">{{ $row->page ?? 'Service' }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-lg-12">
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h5 class="m-0">Add/Update Service</h5>
                        </div>
                        <div class="card-body">
                            <form action="{{ $action }}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="slug" id="{{ isset($row->id) ? 'slug1' : 'slug' }}" class="form-control" value="{{ isset($row->slug) ? $row->slug : old('slug') }}" placeholder="Service Slug" >
                                            @error('slug')
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Display in footer menu</label>
                                            <select class="form-control" name="footer_menu">
                                                <option value="0" {{ isset($row->footer_menu) ? ($row->footer_menu == 0 ? 'selected' : '') : null }} >No</option>
                                                <option value="1" {{ isset($row->footer_menu) ? ($row->footer_menu == 1 ? 'selected' : '') : null }} >Yes</option>
                                            </select>
                                            @error('footer_menu')
                                            <div class="error text-danger">{{ $errors->first('footer_menu') }}</div>
                                            @enderror
                                        </div>
                                    </div>
                                </div>
