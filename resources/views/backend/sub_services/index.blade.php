                ...
                <div class="col-lg-12">
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h5 class="m-0">Sub Services</h5>
                        </div>
                        <div class="card-body">
                            <div class="d-flex justify-content-between mb-2">
                                @can('Sub service Create')
                                <div class="btn-group">
                                    <a href="{{ route('sub_services.create') }}" class="btn btn-primary btn-sm">Add
                                        Sub Service</a>
                                </div>
                                @endcan
                            </div>
                            <table class="table table-bordered">
                                ...
                                @forelse($pages as $page)
                                    <tr>
                                        ...
                                        <td>{{ $page->name }}</td>
                                        <td><a href="{{route('sub_services.update.index.status',$page->id)}}"
                                               class="bdg-index-status">{!! indexStatus($page->index_status) !!}</a></td>
                                        <td><a href="{{route('sub_services.update.status',$page->id)}}"
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                @can('Sub service Create')
                                                <a href="{{ route('sub_services.edit', $page->id) }}"
                                                   class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                                                @endcan
                                                @isset($page->service->slug)
                                                        <a href="{{ route('single.sub_service',[ 'service' => $page->service->slug, 'sub_service' => $page->sub_slug]) }}"
                                                           class="btn btn-default btn-sm ml-2"><i class="fa fa-eye"></i></a>
                                                    @endisset

                                                    @can('Sub service Delete')
